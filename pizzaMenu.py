...
class Pizza(QMainWindow):
    def __init__(win, self):
        ...
        def create_pizzaBox(row, column, displayed_image, label_1_text, label_2_text, label_3_text, label_price_text):
            #----- Group Box -----
            groupBox = QGroupBox(self.scrollAreaWidgetContents)
            groupBox.setSizePolicy(self.fixedSizePolicy)
            ...
            cursor = conn.cursor()
            result = cursor.execute(f"SELECT * FROM pizza_photos WHERE item_id == '{displayed_image + 1}'").fetchone()

            #----- Pizza Image -----
            self.pizzaImage = QLabel(groupBox)
            self.pizzaImage.setGeometry(0, 0, 180, 180)
            pmPizzaImage = QPixmap()
            pmPizzaImage.loadFromData(result[1])
            self.pizzaImage.setPixmap(pmPizzaImage)

            verticalLayout.addWidget(self.pizzaImage)
            #-----------------------
            ...
            pushButton_2.clicked.connect(lambda: add_to_basket(self, [pmPizzaImage, label_1.text(), label_2.text(), label_3.text(), label_price.text()]))
...
#---------------------------------------------------------------------
        def create_pizzas_box():
            pizzaCol = 0
            pizzaRw = 0

            for i in range(33):#33 it's pizza block count
                item_info: str
                ingradients: str

                try:
                    item_info = requests.get("http://127.0.0.1:4444/pizza").json()
                    ingradients = item_info[f"{i}"]["item_ingradients"].strip()
                except:
                    item_info = {
                        "item_name" : "ERROR!",
                        "item_ingradients" : "ERROR!",
                        "item_size" : "ERROR!",
                        "item_price" : "ERROR!",
                    }
                ...
                ingradients_adapted = "".join(pizzaIngredientsList)

                try:
                    if pizzaCol < 4:
                        create_pizzaBox(pizzaRw, pizzaCol, i, item_info[f"{i}"]["item_name"], ingradients_adapted, item_info[f"{i}"]["item_size"], item_info[f"{i}"]["item_price"])
                        pizzaCol += 1
                    else:
                        create_pizzaBox(pizzaRw, pizzaCol, i, item_info[f"{i}"]["item_name"], ingradients_adapted, item_info[f"{i}"]["item_size"], item_info[f"{i}"]["item_price"])
                        pizzaRw += 1
