...
class Sushi(QMainWindow):
    def __init__(win, self):
        super(Sushi, win).__init__()
        ...
        #------------CREATE BUTTON FUNCTION-----------------------SUSHI

        def create_sushiBox(row, column, displayed_image : str, label_1_text, label_2_text, label_3_text, label_price_text):
            #----- Group Box -----
            ...
            cursor = conn.cursor()
            result = cursor.execute(f"SELECT * FROM sushi_photos WHERE item_id == '{displayed_image + 1}'").fetchone()


            #----- Pizza Image -----
            self.sushiImage = QLabel(groupBox)
            self.sushiImage.setGeometry(0, 0, 180, 180)
            pmSushiImage = QPixmap()
            pmSushiImage.loadFromData(result[1])
            self.sushiImage.setPixmap(pmSushiImage)


            verticalLayout.addWidget(self.sushiImage)
            #-----------------------
            ...
            pushButton_2.clicked.connect(lambda: add_to_basket(self, [pmSushiImage, label_1.text(), label_2.text(), label_3.text(), label_price.text()]))
        ...
        def create_sushi_box():

            sushiCol = 0
            sushiRw = 0

            #----INGREDIENTS----
            for i in range(106): #106 it's sushi block count

                item_info = requests.get("http://127.0.0.1:4444/sushi").json()
                ...
                tempSushiIngredientList = []


                for el in di:
                    # if i != "\n":
                    new = "● " + el.strip().capitalize() + "\n"
                    tempSushiIngredientList.append(new)

                sushiIngredientsList = []

                for j in tempSushiIngredientList:
                    if j == "● \n":
                        let = j.replace("● ", "")
                        sushiIngredientsList.append(let)
                    else:
                        sushiIngredientsList.append(j)

                norm = "".join(sushiIngredientsList)

                try:
                    if sushiCol < 4:
                        create_sushiBox(sushiRw, sushiCol, i, item_info[f"{i}"]["item_name"], norm, item_info[f"{i}"]["item_size"], item_info[f"{i}"]["item_price"])
                        sushiCol += 1
                    else:
                        create_sushiBox(sushiRw, sushiCol, i, item_info[f"{i}"]["item_name"], norm, item_info[f"{i}"]["item_size"], item_info[f"{i}"]["item_price"])
                        sushiRw += 1
