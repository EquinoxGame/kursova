...
class Euro(QMainWindow):
    def __init__(win, self):
        super(Euro, win).__init__()
        ...
        def add_to_basket(parent, item_info):
            ...
            item_name.setStyleSheet("background: green;")
        ...
        #------------CREATE BUTTON FUNCTION-----------------------EURO
        def create_euroBox(row, column, displayed_image : str, label_1_text, label_2_text, label_3_text, label_price_text):
            #----- Group Box -----
            ...
            cursor = conn.cursor()
            result = cursor.execute(f"SELECT * FROM euro_photos WHERE item_id == '{displayed_image + 1}'").fetchone()


            #----- Pizza Image -----
            self.euroImage = QLabel(groupBox)
            self.euroImage.setGeometry(0, 0, 180, 180)
            pmEuroImage = QPixmap()
            pmEuroImage.loadFromData(result[1])
            self.euroImage.setPixmap(pmEuroImage)


            verticalLayout.addWidget(self.euroImage)
            #-----------------------
            ...
            pushButton_2.clicked.connect(lambda: add_to_basket(self, [pmEuroImage, label_1.text(), label_2.text(), label_3.text(), label_price.text()]))
        ...
        def create_euro_box():
            ...
            for i in range(73):#73 it's euro block count
                item_info = requests.get("http://127.0.0.1:4444/euro").json()
                ...
                try:
                    if euroCol < 4:
                        create_euroBox(euroRw, euroCol, i, item_info[f"{i}"]["item_name"], norm, item_info[f"{i}"]["item_size"], item_info[f"{i}"]["item_price"])
                        euroCol += 1
                    else:
                        create_euroBox(euroRw, euroCol, i, item_info[f"{i}"]["item_name"], norm, item_info[f"{i}"]["item_size"], item_info[f"{i}"]["item_price"])
                        euroRw += 1
